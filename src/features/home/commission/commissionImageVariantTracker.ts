...
const normalizeImageUrl = (url: string, win: Window) => {
  if (!url) return ''
...
const parseSrcSetCandidates = (srcSet: string, win: Window): SrcSetCandidate[] => {
  if (!srcSet) return []
...
const detectVariantFromQueryParam = (
  sourceUrl: string,
  win: Window,
): '768' | '960' | '1280' | null => {
  try {
...
export const createCommissionImageVariantTracker = (
