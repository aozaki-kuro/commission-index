...
describe('CommissionSearch', () => {
  ...
  it('requests deferred section entries when character search starts with pending section templates', async () => {
    const dispatchEventSpy = vi.spyOn(window, 'dispatchEvent')
    document.body.innerHTML = `
      <div
        data-commission-view-panel="character"
        ...
        data-stale-visibility="hidden"
      >
        <section id="section-alpha" data-character-section="true" data-character-status="active" data-section-entries-loaded="false">
          <div data-commission-entry="true" data-character-section-id="section-alpha" data-commission-search-key="section-alpha::20240101_alpha"></div>
          <div data-section-entries-container="true"></div>
          <template data-section-entries-template="true">
            <div data-commission-entry="true" data-character-section-id="section-alpha" data-commission-search-key="section-alpha::20240102_beta"></div>
          </template>
        </section>
      </div>
    `
    ...
    const entries: CommissionSearchEntrySource[] = [
      {
        id: 1,
        domKey: 'section-alpha::20240101_alpha',
        searchText: 'alpha sample',
        searchSuggest: 'Character\tAlpha',
      },
      {
        id: 2,
        domKey: 'section-alpha::20240102_beta',
        searchText: 'beta sample',
        searchSuggest: 'Keyword\tbeta',
      },
    ]
    ...
    try {
      ...
      await waitFor(() => {
        expect(
          dispatchEventSpy.mock.calls.some(
            ([event]) =>
              event instanceof Event && event.type === SECTION_ENTRIES_LOAD_REQUEST_EVENT,
          ),
        ).toBe(true)
      })
    } finally {
  ...
  it('rebuilds the character DOM mapping after deferred section entries are mounted', async () => {
    document.body.innerHTML = `
      <div
        data-commission-view-panel="character"
        data-commission-view-active="true"
      ...
      >
        <section id="section-alpha" data-character-section="true" data-character-status="active" data-section-entries-loaded="false">
          <div
            data-commission-entry="true"
            data-character-section-id="section-alpha"
            data-commission-search-key="section-alpha::20240101_alpha"
          ></div>
          <div data-section-entries-container="true"></div>
          <template data-section-entries-template="true">
            <div
              data-commission-entry="true"
              data-character-section-id="section-alpha"
              data-commission-search-key="section-alpha::20240102_beta"
            ></div>
          </template>
        </section>
      </div>
    ...
    document
      .querySelector<HTMLElement>('[data-section-entries-container="true"]')
      ?.insertAdjacentHTML(
        'beforeend',
        `
          <div
    ...
    document
      ...
      ?.setAttribute('data-section-entries-loaded', 'true')
    document.querySelector('template[data-section-entries-template="true"]')?.remove()

    window.dispatchEvent(new Event(SECTION_ENTRIES_LOADED_EVENT))

    await waitFor(() => {
      const deferredEntry = document.querySelectorAll<HTMLElement>(
        '[data-commission-entry="true"]',
      )[1]
      expect(deferredEntry?.classList.contains('hidden')).toBe(false)
