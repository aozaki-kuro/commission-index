...
export default function CommissionSearchHelpModal({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent
        overlayClassName="bg-gray-900/30 backdrop-blur-[2px] dark:bg-black/55 data-[state=open]:animate-[dialog-overlay-in_200ms_ease-out] data-[state=closed]:animate-[dialog-overlay-out_150ms_ease-in]"
        className="w-full max-w-md rounded-2xl border border-gray-300/80 bg-white/95 p-5 text-sm text-gray-700 shadow-[0_16px_50px_rgba(0,0,0,0.16)] backdrop-blur-sm data-[state=closed]:animate-[dialog-content-out_150ms_ease-in] data-[state=open]:animate-[dialog-content-in_200ms_ease-out] md:text-base dark:border-gray-700 dark:bg-black/90 dark:text-gray-300"
      >
        <DialogTitle className="text-base font-bold text-gray-900 md:text-lg dark:text-gray-100">
          Search Help
        </DialogTitle>
        <DialogDescription className="sr-only">
          Search syntax and examples for filtering commission entries.
        </DialogDescription>

        <div className="mt-3 space-y-3 text-gray-700 dark:text-gray-300">
          <p className="text-xs md:text-sm">Type one or more keywords to filter commissions.</p>
